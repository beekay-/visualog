# Original source: https://github.com/blech/moves-geojson
from app import app
import logging
import os
import sys
...
def list():
    ...
    summary = Moves.user_summary_daily(pastDays=14, access_token=session['token'])
    summary.reverse()
...
def get_month_range(first_date, last_date=None, excluding=None):
    ...
    while cursor.year > first.year or cursor.month >= first.month and cursor.year >= 2017:
...
def make_summaries(day):
    returned = {}
    lookup = {'running': 'running', 'cycling': 'cycling', 'car':'car', 'walking':'walking', 'train':'train', 'bus':'bus', 'airplane':'airplane', 'transport':'transport', 'underground':'underground', 'tram':'tram', 'tennis':'tennis'}
...
def geojson_move(segment):
    features = []
    lookup = {'running': 'running', 'cycling': 'cycling', 'car':'car', 'walking':'walking', 'train':'train', 'bus':'bus', 'airplane':'airplane', 'transport':'transport', 'underground':'underground', 'tram':'tram', 'tennis':'tennis'}
    stroke = {'walking': '#00d45a', 'running': '#93139a', 'cyc': '#00ceef', 'car':'#000000', 'train': '#000', 'bus':'#000000', 'airplane': '#000000', 'underground':'#000000', 'cycling':'#000000', 'transport':'#000000', 'tram':'#000000', 'tennis':'#000000'}
